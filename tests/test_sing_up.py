...
from helpers import random_mail


class TestSingUp:
    def test_sing_up_passed_login(self, browser):
    ...
    def test_sing_up_no_valid_mail_not_login(self, browser):
        browser.find_element(*SING_IN).click() 
        WebDriverWait(browser, 3).until(EC.element_to_be_clickable(NO_ACC))  
        browser.find_element(*NO_ACC).click()
    ...
    def test_sing_up_exist_user_not_valid(self, browser):
        browser.find_element(*SING_IN).click() 
        WebDriverWait(browser, 3).until(EC.element_to_be_clickable(NO_ACC))  
        browser.find_element(*NO_ACC).click()
