...
def test_logout_in_ex_acc_logout(browser):
    browser.find_element(*SING_IN).click()  
    WebDriverWait(browser, 3).until(EC.element_to_be_clickable(EMAIL))
    browser.find_element(*EMAIL).send_keys('test@tests.test')
    browser.find_element(*PASSWORD).send_keys('parol')
    ...
    assert len(browser.find_elements(*AVATAR_USER)) == 0 and len(browser.find_elements(*USER_NAME)) == 0 and browser.find_element(*SING_IN).is_displayed()
